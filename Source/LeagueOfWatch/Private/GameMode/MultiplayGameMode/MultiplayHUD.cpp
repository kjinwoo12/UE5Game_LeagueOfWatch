...
#include "GameMode/MultiplayGameMode/MultiplayHUD.h"
...
void AMultiplayHUD::AllPlayersReady_Implementation()
{
	TArray<UUserWidget*> foundWidgets;
	UWidgetBlueprintLibrary::GetAllWidgetsWithInterface(GetWorld(), foundWidgets, UMultiplayEventListener::StaticClass(), true);
	for (UUserWidget* widget : foundWidgets)
	{
		IMultiplayEventListener::Execute_AllPlayersReady(widget);
	}
}