...
#include "Feedback/ContextEffects/AnimNotify_ContextEffects.h"
#include "Engine/World.h"
#include "Kismet/GameplayStatics.h"
#include "Feedback/ContextEffects/ContextEffectsInterface.h"
#include "Feedback/ContextEffects/ContextEffectsSubsystem.h"
#include "Feedback/ContextEffects/ContextEffectsLibrary.h"
#include "NiagaraFunctionLibrary.h"

FString UAnimNotify_ContextEffects::GetNotifyName_Implementation() const
{
...
void UAnimNotify_ContextEffects::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation,
	const FAnimNotifyEventReference& EventReference)
{
	...
	if (MeshComp)
	{
		...
		if (AActor* OwningActor = MeshComp->GetOwner())
		{
			...
			if (OwningActor->Implements<UContextEffectsInterface>())
			{
			...
			for (const auto Component : OwningActor->GetComponents())
			{
				if (Component)
				{
					...
					if (Component->Implements<UContextEffectsInterface>())
					{
			...
			for (UObject* ContextEffectImplementingObject : ContextEffectImplementingObjects)
			{
				if (ContextEffectImplementingObject)
				{
					...
					IContextEffectsInterface::Execute_AnimMotionEffect(ContextEffectImplementingObject,
			...
			if (bPreviewInEditor)
			{
				...
				if (World && World->WorldType == EWorldType::EditorPreview)
				{
					...
					{
						// Check if it is in fact a UContextEffectLibrary type
						if (UContextEffectsLibrary* EffectLibrary = Cast<UContextEffectsLibrary>(EffectsLibrariesObj))
						{
...
#if WITH_EDITOR
void UAnimNotify_ContextEffects::ValidateAssociatedAssets()
{
	Super::ValidateAssociatedAssets();
}

void UAnimNotify_ContextEffects::SetParameters(FGameplayTag EffectIn, FVector LocationOffsetIn, FRotator RotationOffsetIn,
	FContextEffectAnimNotifyVFXSettings VFXPropertiesIn, FContextEffectAnimNotifyAudioSettings AudioPropertiesIn,
	bool bAttachedIn, FName SocketNameIn, bool bPerformTraceIn, FContextEffectAnimNotifyTraceSettings TracePropertiesIn)
{
	Effect = EffectIn;
